...
const register = async (username, pass) => {
  ...
  if (!userElem) throw new AppError("Invalid!!!", 409);
...
const login = async (username, pass) => {
  ...
  if (!users) throw new AppError("Invalid!!!", 401);
  if (!(await verifyPassword(pass, users.passwordHash)))
    throw new AppError("Json Web Token Error!!!", 409);
...
const getMe = (id) => {
  ...
  if (!idUser) throw new AppError("Invalid!!!");
  return idUser;
};
